...
func FormatValidationError(err error) map[string]string {
	...
	if validationError, ok := errors.AsType[validator.ValidationErrors](err); ok {
		for _, e := range validationError {
			...
			case "eqfield":
				out[field] = fmt.Sprintf("The %s field does not match the %s field.", field, param)
			case "uuid":
				out[field] = fmt.Sprintf("The %s field must be a valid UUID.", field)
			default:
